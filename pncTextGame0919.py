...
import sys # 시스템 임포트
import random as rd # 난수 생성용 랜덤 임포트
import copy # 클래스 복제용 카피 임포트
########################################exe 생성 방법(pyinstaller)##################################
# pyinstaller --onefile --windowed --icon=icon.ico pncTextGame.py
########################################파이 게임 및 텍스트#########################################
...
FONT_50B = pygame.font.SysFont("malgungothic", 50, bold=True)
FONT_40 = pygame.font.SysFont("malgungothic", 40)
FONT_40B = pygame.font.SysFont("malgungothic", 40, bold=True)
FONT_30 = pygame.font.SysFont("malgungothic", 30)
FONT_30B = pygame.font.SysFont("malgungothic", 30, bold=True)
FONT_24 = pygame.font.SysFont("malgungothic", 24)
FONT_24B = pygame.font.SysFont("malgungothic", 24, bold=True)
FONT_22 = pygame.font.SysFont("malgungothic", 22)
FONT_22B = pygame.font.SysFont("malgungothic", 22, bold=True)
FONT_20 = pygame.font.SysFont("malgungothic", 20)
FONT_20B = pygame.font.SysFont("malgungothic", 20, bold=True)
FONT_18 = pygame.font.SysFont("malgungothic", 18)
FONT_18B = pygame.font.SysFont("malgungothic", 18, bold=True)
FONT_16 = pygame.font.SysFont("malgungothic", 16)
FONT_16B = pygame.font.SysFont("malgungothic", 16, bold=True)
FONT_14 = pygame.font.SysFont("malgungothic", 14)
FONT_14B = pygame.font.SysFont("malgungothic", 14, bold=True)
FONT_12 = pygame.font.SysFont("malgungothic", 12)
FONT_12B = pygame.font.SysFont("malgungothic", 12, bold=True)
# 텍스트
...
class PlayableDoll:
    ...
    def act_normalAttack(self, other, cursor, color, turn):
        ...
        if nan_avd < other.avd: # 적 회피 판정
            SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 공격은 빗나갔다...", True, color))
        ...
        if nan_crt < self.crt: # 치명타 판정
            ...
            SYS_BATTLE_LOG.append(FONT_22.render(f"크리티컬 히트! {self.name}의 공격은 {other.name}에게 {sys_damage}의 데미지를 입혔다!", True, color))
            return
        else: # 기본 공격 판정
            ...
            if sys_damage < min_dam:
                sys_damage = min_dam
            other.hp -= sys_damage # 적 체력 감소
            SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 공격은 {other.name}에게 {sys_damage}데미지를 입혔다.", True, color))
            return
    # 행동 선택 충전 함수
    def act_hashCharge(self, color, type):
        ...
        if type == "chg": # 충전시 연산 회복
            if(self.mp < self.maxmp):
                ...
                SYS_BATTLE_LOG.append(FONT_22.render(f"충전을 통해 {self.name}의 연산량을 {sys_hash}만큼 확보했다.", True, color))
        #기타 옵션 추가...예정
        self.mp += sys_hash # 회복
        ...
        return
    # 선행 패시브 스킬 함수
    def act_passiveBfSkill(self, other, cursor, color, turn): # 패시브 스킬 구현부
        # 글로벌 변수 호출
        ...
        if self.name == None:
            ...
            if self.value == 0: # 다용도 변수가 0이 될 때 마다
                dam_psk = int(sys_damage * 0.6) # 스킬 배수
                dam_psk = int((dam_psk - other.dfs) * (1 + self.dmg / 100) * (1- other.prt / 100)) # 방어 계산
                if dam_psk < 0:
                    dam_psk = 1
                other.hp -= dam_psk # 피해 적용
                SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '축적·들뜸 중첩' 발동. 폭축으로 적에게 {dam_psk}의 데미지를 주었다.", True, color))
            self.value += 1 # 매 턴마다 변수 값 증가
            if self.value == 4: # 5턴마다 쿨타임 초기화
                self.value = 0
        elif self.name == "드 레이시":
            if self.hp < self.maxhp:
                dam_psk = int(sys_damage * 0.2) # 스킬 배수
                self.hp += dam_psk # 회복
                if self.hp > self.maxhp:
                    self.hp = self.maxhp
                SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '자체 대상 작용' 발동. 자체 연결로 생명력을 {dam_psk}회복했다.", True, color))
        elif self.name == "클로토":
            if (rd.random() * 100) < 40:
                if self.value < 5:
                    self.value += 1
                    SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '추모의 나비' 발동. 유령 나비({self.value})로 방어력이 {self.value * 2}증가했다.", True, color))
            self.dfs = self.defaultdfs + (self.value * 2)
        elif self.name == "메메코":
            if turn == 200: # 200턴 째가 되면 자살
                self.hp = 0
        pass
    # 후행 패시브 스킬 함수
    def act_passiveAfSkill(self, other, cursor, color, turn): # 패시브 스킬 구현부
        # 글로벌 변수 호출
        global SYS_BATTLE_LOG # 전투 로그
        global SYS_TURN_PLAYER #현재 턴 소유주 호출
        sys_damage = int((self.atk * (0.9 + rd.random() * 0.2))) # 공격력의 90%~110% 사이의 랜덤 데미지
        if self.name == None:
            ...
            if (rd.random() * 100) < 10 and cursor < 2: # 랜덤 난수 생성
                SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '데이터 침식' 발동. 적의 턴을 침식시켰다.", True, color))
                ...
                return
            if self.check == True:
                if (rd.random() * 100) < 50 and cursor < 2: # 랜덤 난수 생성
                    self.check = False
                    SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '데이터 침식' 발동! 적의 턴을 침식시켰다!", True, color))
                    if SYS_TURN_PLAYER == 1: # 턴 변경 방지
                        SYS_TURN_PLAYER = 2
            ...
            if cursor == 0 and (rd.random() * 100) < 25: # 랜덤 난수 생성
                dam_psk = [int(((self.atk * (0.8 + rd.random() * 0.4)) * 0.12) * (1 + self.dmg / 100) * (1 - other.prt / 100)) for i in range(3)]
                other.hp -= sum(dam_psk) # 피해 적용
                SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '피어나는 죽음' 발동. 삼점사로 적에게 {sum(dam_psk)}={dam_psk}의 데미지를 주었다.", True, color))
        elif self.name == "드 레이시":
            if self.value > 0 : # 패시브에서 궁극기 체크 발동
                self.value -= 1 # 횟수 감소
                sys_damage = int((self.atk * (0.8 + rd.random() * 0.4))) # 공격력의 90%~110% 사이의 랜덤 데미지
                dam_usk = int((sys_damage * 1.5 - other.dfs) * (1 + self.dmg / 100) * (1- other.prt / 100)) # 방어 계산
                shd_usk = sys_damage
                other.hp -= dam_usk
                self.hp += shd_usk
                SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '철심의 섬광' 발동!! 전자파로 적에게 {dam_usk}의 데미지를 주고 쉴드를 {shd_usk}얻었다!!", True, color))
            if other.name == "페르시카-집도" or other.name =="페르시카":
                if (rd.random() * 100) < 20: # 랜덤 난수 생성
                    dam_psk = int(sys_damage * 0.7) # 스킬 배수
                    SYS_BATTLE_LOG.append(FONT_22.render(f"페시 양, 저는 교수님이 정말 좋아요!(페르시카에게 {dam_psk}의 데미지를 주었다.)", True, color))
        elif self.name == "보름":
            if cursor == 0:
                self.value += 1 # 평타 선택시 히트수 증가
            if self.value >= 4: # 5스택 쌓이면
                cnt_psk = self.value // 4
                cnt_rmn = self.value % 4
                for i in range(cnt_psk):
                    dam_psk = int(((((self.atk * (0.8 + rd.random() * 0.4)) * 0.5) + ((sys_damage * 0.3) * self.buffduration))- other.dfs) * (1 + self.dmg / 100) * (1- other.prt / 100))
                    if dam_psk < 0:
                        dam_psk = 1
                    other.hp -= dam_psk
                    SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '명시의 칼날' 발동. 명시의 칼날로 적에게 {dam_psk}의 데미지를 주었다.", True, color))
                self.value = cnt_rmn
        elif self.name == "클로토":
            if self.buffduration > 0 and cursor != 2:
                self.buffduration -= 1
                dam_ask = int((((self.dfs * (0.9 + rd.random() * 0.2)) * 2) - other.dfs) * (1 + self.dmg / 100) * (1- other.prt / 100)) # 데미지 계산
                other.hp -= dam_ask # 피해 적용
                SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '운명 재단'의 방어력 비례 피해 발생! 적에게 {dam_ask}의 데미지를 주었다!", True, color))
        pass
    ...
    def act_activeSkill(self, other, cursor, color, turn): # 액티브 스킬 구현부
        ...
        if self.name == None:
            ...
            SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '천공·벡터 발산' 발동! 폭축으로 적에게 {sum(dam_ask)}={dam_ask}의 데미지를 주었다!", True, color))
        elif self.name == "안토니나":
            self.check = True # 패시브 강화
            dam_ask = int(((sys_damage * 2.5)- other.dfs) * (1 + self.dmg / 100) * (1- other.prt / 100))
            other.hp -= dam_ask # 피해 적용
            SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '연쇄감염' 발동! 트로이 공격으로 적에게 {dam_ask}의 데미지를 주었다!", True, color))
            if (rd.random() * 100) < 70: # 랜덤 난수 생성
                SYS_BATTLE_LOG.append(FONT_22.render(f"이어서 '데이터 침식' 발동! 적의 턴을 침식시켰다!", True, color))
                if SYS_TURN_PLAYER == 1: # 턴 변경 방지
                    SYS_TURN_PLAYER = 2
                elif SYS_TURN_PLAYER ==2:
                    SYS_TURN_PLAYER = 1
            ...
            other.hp -= dam_ask # 피해 적용
            SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '밤하늘의 천둥' 발동! 은멸 유탄으로 적에게 {dam_ask}의 데미지를 주었다!", True, color))
        elif self.name == "드 레이시":
            self.buffduration += 4 # 패시브 기회 3회 추가 패시브로 가져가기
        elif self.name == "보름":
            x = rd.randint(3, 5) # 3~5회 지정
            self.value += x # 패시브 히트수 계산
            dam_ask = [int(((self.atk * (0.9 + rd.random() * 0.2)) * 0.6 - other.dfs) * (1 + self.dmg / 100) * (1 - other.prt / 100)) for i in range(x)]
            dam_ask = sys_negativeDamage(dam_ask, int(sys_damage * 0.05)) # 방어력 때문에 음수 발생하는 것 방지
            other.hp -= sum(dam_ask)
            SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '추적의 거울' 발동! 룬의 힘으로 적에게 {sum(dam_ask)}={dam_ask}의 데미지를 주었다!", True, color))
        elif self.name == "클로토":
            dam_ask = int((((self.dfs * (0.9 + rd.random() * 0.2)) * 5) - other.dfs) * (1 + self.dmg / 100) * (1- other.prt / 100)) # 데미지 계산
            other.hp -= dam_ask # 피해 적용
            self.buffduration += 3
            SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '운명 재단' 발동! 심판으로 적에게 {dam_ask}의 데미지를 주었다!", True, color))
        elif self.name == "메메코":
            other.hp -= other.maxhp // 10 # 적 체력 절대치 감소
            SYS_BATTLE_LOG.append(FONT_22.render(f"？？？의 '￥％, ￥＃＊＆!' 발동! ￥＊＆＃으로 적에게 {other.maxhp // 10}의 데미％￥＃＆￥＆...!", True, color))
        pass
    # 행동 선택 궁극기 함수(선택 발동)
    def act_ultimateSkill(self, other, cursor, color, turn): # 궁극기 구현부
        ...
        if self.name == None:
            ...
            dam_usk = int((sys_damage * 4.5) * (1 + self.dmg / 100) * (1 - other.prt / 100))
            other.hp -= dam_usk
            SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 폭연·선형 분열 발동!! 적에게 {dam_usk}의 데미지를 주었다!!", True, color))
        elif self.name == "안토니나":
            if True: # 랜덤 난수 생성
                self.value += 3 # 패시브 기회 3회 추가
                SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '글로벌 해킹' 발동!! 적의 턴을 침식시켰다!!", True, color))
                if SYS_TURN_PLAYER == 1: # 턴 변경 방지
                    SYS_TURN_PLAYER = 2
                elif SYS_TURN_PLAYER ==2:
                    SYS_TURN_PLAYER = 1
            ...
            dam_usk = [int(((self.atk * (0.8 + rd.random() * 0.4)) * 1.3 - other.dfs) * (1 + self.dmg / 100) * (1 - other.prt / 100)) for i in range(5)]
            dam_usk = sys_negativeDamage(dam_usk, int(sys_damage * 0.05)) # 방어력 때문에 음수 발생하는 것 방지
            other.hp -= sum(dam_usk) # 피해 적용
            SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '데이브레이커' 발동!! 공습 지원으로 적에게 {sum(dam_usk)}={dam_usk}의 데미지를 주었다!!", True, color))
        elif self.name == "드 레이시":
            self.value += 3 # 패시브 기회 3회 추가 패시브로 가져가기
        elif self.name == "보름":
            x = rd.randint(7, 10) # 7~10회 지정
            self.value += x # 패시브 히트수 계산
            if self.buffduration < 3:
                self.buffduration += 1
            dam_ask = [int(((self.atk * (0.8 + rd.random() * 0.4)) * 0.5 - other.dfs) * (1 + self.dmg / 100) * (1 - other.prt / 100)) for i in range(x)]
            dam_ask = sys_negativeDamage(dam_ask, int(sys_damage * 0.05)) # 방어력 때문에 음수 발생하는 것 방지
            other.hp -= sum(dam_ask)
            SYS_BATTLE_LOG.append(FONT_22.render(f"{self.name}의 '명월청천' 발동!! 룬의 힘으로 적에게 {sum(dam_ask)}={dam_ask}의 데미지를 주었다!!", True, color))
        elif self.name == "클로토":
            self.nuffduration += 4
        elif self.name == "메메코":
            other.hp -= other.maxhp // 3 # 적 체력 절대치 감소
            other.maxhp -= other.maxhp // 3 # 적 최대체력 절대치 감소
            SYS_BATTLE_LOG.append(FONT_22.render(f"？？？의 '￥＃＆％＊＆￥!' 발＃!! ￥＊＆＃으로 적￥ {other.maxhp // 5 // 10}％ ￥＃＆지＊＃다...!!", True, color))
        pass
...
# 턴 중립 함수
def act_neutralEffect(p1, p2, cursor, color, turn):
        global SYS_BATTLE_LOG # 전투 로그
        global SYS_TURN_PLAYER #현재 턴 소유주 호출
        sys_damage = int((p1.atk * (0.9 + rd.random() * 0.2))) # 공격력의 90%~110% 사이의 랜덤 데미지
        if p1.name == None:
            pass
        elif p1.name == "드 레이시":
            if p1.buffduration > 0 : # 패시브에서 액티브 체크 발동
                p1.buffduration -= 1 # 횟수 감소
                dam_ask = int((sys_damage * 0.8 - p2.dfs) * (1 + p1.dmg / 100) * (1- p2.prt / 100)) # 방어 계산
                shd_ask = int(sys_damage * 0.5)
                p2.hp -= dam_ask
                p1.hp += shd_ask
                SYS_BATTLE_LOG.append(FONT_22.render(f"{p1.name}의 '구조해석 연결' 발동! 전자파로 적에게 {dam_ask}의 데미지를 주고 체력을 {shd_ask}회복했다!", True, color))
        elif p1.name == "클로토":
            if p1.nuffduration > 0 :
                p1.nuffduration -= 1
                dam_usk = int(((((p1.dfs + p1.value * 3) * (0.9 + rd.random() * 0.2)) * 3) - p2.dfs) * (1 + p1.dmg / 100) * (1- p2.prt / 100)) # 데미지 계산
                p2.hp -= dam_usk # 피해 적용
                SYS_BATTLE_LOG.append(FONT_22.render(f"{p1.name}의 '영혼의 아리아' 발동!! 유령나비로 적에게 {dam_usk}의 데미지를 주었다!", True, color))
                if p1.nuffduration == 0:
                    p1.value = 0
        pass
# 플레이어 턴 시스템 함수
def sys_playerTurn(player, opponent, cursor, color, turn): # 플레이어 턴의 행동 처리
    ...
    if cursor == 0: # 일반 공격
        ...
        SYS_BATTLE_LOG.append(FONT_22.render(f"{player.name}의 턴을 바로 종료 합니다.", True, color))
    player.act_passiveAfSkill(opponent, cursor, color, turn) # 후행 패시브 스킬
    act_neutralEffect(pc1_Doll, pc2_Doll, cursor, COL_RED, turn) # 턴 중립 효과 발동
    act_neutralEffect(pc2_Doll, pc1_Doll, cursor, COL_BLUE, turn) # 턴 중립 효과 발동
    ### 턴 종료 수치 증가 ###
...
def sys_statDisplay(player, pnum, color): # 플레이어 클래스, 플레이어 ID(SYS_TURN_PLAYER)
    ...
    SYS_STATUS.append(FONT_20.render(f"      현재 생명력 : [ {player.hp} / {player.maxhp} ]", True, COL_WHITE))
    ...
    SYS_STATUS.append(FONT_20.render(f"      현재 연산량 : [ {player.mp} / {player.maxmp} ]", True, COL_WHITE))
    ...
    if now > 14:
        ...
        SYS_STATUS.append(FONT_18.render(f"[" + ("■" * now) + ("□" * (20 - now)) + "]", True, COL_MP00))
    SYS_STATUS.append(FONT_22.render(f"공격력 : {player.atk} / 연산력 : {player.hsh}", True, COL_WHITE))
    SYS_STATUS.append(FONT_22.render(f"방어력 : {player.dfs} / 회피율 : {player.avd}%", True, COL_WHITE))
    SYS_STATUS.append(FONT_22.render(f"치명률 : {player.crt}% / 치명피해 : {player.crd}%", True, COL_WHITE))
    SYS_STATUS.append(FONT_22.render(f"고유 스킬 : {player.scost} hr / 궁극 스킬 : {player.ucost} hr", True, COL_WHITE))
...
def sys_eraserDisplay(name, x, y):
    ...
    "드 레이시": pygame.image.load("./data/ers_delacey.png"),
    "보름": pygame.image.load("./data/ers_luna.png"),
    "클로토": pygame.image.load("./data/ers_clotho.png"),
...
# 지정된 좌표 입력 받아 사각형 그리기
def sys_rectDisplay(screen, color, x, y, w, h):
    x = x - w // 2
    y = y - h // 2
...
wr01_PersicaSE = PlayableDoll("페르시카-집도", _hp=1600, _mp=1350, _atk=110, _hsh=110, _dfs=15, _avd=10, _crt=15, _crd=70, _dmg=0, _prt=0, _scost=450, _ucost=1000)
sp01_Antonina  = PlayableDoll("안토니나", _hp=1500, _mp=1700, _atk=90, _hsh=120, _dfs=10, _avd=15, _crt=20, _crd=50, _dmg=0, _prt=0, _scost=500, _ucost=900)
...
md01_Delacey = PlayableDoll("드 레이시", _hp=1600, _mp=1550, _atk=80, _hsh=120, _dfs=15, _avd=10, _crt=15, _crd=50, _dmg=0, _prt=0, _scost=500, _ucost=900)
sp02_Luna = PlayableDoll("보름", _hp=1550, _mp=1800, _atk=90, _hsh=130, _dfs=15, _avd=15, _crt=30, _crd=50, _dmg=0, _prt=0, _scost=400, _ucost=900)
gd01_Clotho = PlayableDoll("클로토", _hp=1800, _mp=1400, _atk=85, _hsh=120, _dfs=20, _avd=15, _crt=5, _crd=50, _dmg=0, _prt=0, _scost=500, _ucost=1200)
# 테스트용 및 초기화용 캐릭터
...
ex01_Memeko = PlayableDoll("메메코", _hp=10000, _mp=10000, _atk=1, _hsh=100, _dfs=10, _avd=1, _crt=1, _crd=100000, _dmg=0, _prt=0, _scost=3000, _ucost=5000)
# 플레이어블 캐릭터 리스트
doll_list = [wr01_PersicaSE, sp01_Antonina, sn01_Clukay, md01_Delacey, sp02_Luna, gd01_Clotho, ex01_Memeko]
# endregion
####################################################################################################
...
def scn_titleMenu(): # 타이틀 씬 스테이지 ID 0
    ...
    while loop_st0 :
        ...
        # 이미지 불러오기
        img_title = pygame.image.load("./data/img_title.png")
        # 이미지 크기를 화면 크기에 맞추기
        img_title = pygame.transform.scale(img_title, (WIN_WIDTH, WIN_HEIGHT))
        WIN_SCREEN.blit(img_title, (0, 0))
        sys_rectDisplay(WIN_SCREEN, COL_BLACK, WIN_WIDTH // 2, WIN_HEIGHT // 4, 600, 100)
        sys_rectDisplay(WIN_SCREEN, COL_BLACK, WIN_WIDTH // 2, WIN_HEIGHT // 2 + 115, 400, 60)
        sys_rectDisplay(WIN_SCREEN, COL_BLACK, WIN_WIDTH // 2, WIN_HEIGHT // 2 + 185, 400, 60)
        sys_rectDisplay(WIN_SCREEN, COL_BLACK, WIN_WIDTH // 2, WIN_HEIGHT // 2 + 255, 400, 60)
        sys_rectDisplay(WIN_SCREEN, COL_BLACK,  WIN_WIDTH // 4 , WIN_HEIGHT * 0.97, 550, 50)
        # 텍스트 출력
        sys_textDisplay(TXT_MENU_TITLE, WIN_WIDTH // 2, WIN_HEIGHT // 4) # 메인 타이틀
        sys_textDisplay(TXT_MENU_START, WIN_WIDTH // 2, WIN_HEIGHT // 2 + 115) # 게임 시작
        sys_textDisplay(TXT_MENU_MANUEL, WIN_WIDTH // 2, WIN_HEIGHT // 2 + 185) # 게임 설명
        sys_textDisplay(TXT_MENU_QUIT, WIN_WIDTH // 2, WIN_HEIGHT // 2 + 255) # 게임 종료
        sys_textDisplay(TXT_MENU_INFO, WIN_WIDTH // 4 , WIN_HEIGHT * 0.97) # 게임 정보
        # 커서 이동
        if sys_cursor_v == 0:
            sys_textDisplay(TXT_SELECT_CURSOR, WIN_WIDTH // 2, WIN_HEIGHT // 2 + 115)
        elif sys_cursor_v == 1:
            sys_textDisplay(TXT_SELECT_CURSOR, WIN_WIDTH // 2, WIN_HEIGHT // 2 + 185)
        elif sys_cursor_v == 2:
            sys_textDisplay(TXT_SELECT_CURSOR, WIN_WIDTH // 2, WIN_HEIGHT // 2 + 255)
        # 사용자 입력 이벤트 처리
...
def scn_battleMain(): # 메인 전투 화면 씬 스테이지 ID 2
    ...
    if 50 <= nan_turn:
        ...
        SYS_BATTLE_LOG.append(FONT_22.render(f"{pc1_Doll.name}의 선제 공격({int(nan_turn)})", True, COL_RED))
    else:
        SYS_TURN_PLAYER = 2 # 플레이어2 선제공격
        SYS_BATTLE_LOG.append(FONT_22.render(f"{pc2_Doll.name}의 선제 공격({int(nan_turn)})", True, COL_BLUE))
    ...
    while loop_st2:
        ...
        for log in SYS_BATTLE_LOG:
            ...
            line_space += 25 # 출력 행간
        if len(SYS_BATTLE_LOG) > 9: # 로그 행 제한
                ...
                if event.type == pygame.KEYDOWN:
                    if (pc1_Doll.sys_aliveCheck() and pc2_Doll.sys_aliveCheck()):
                        ...
                        if event.key == pygame.K_z: # 행동 선택 결과
                            SYS_BATTLE_LOG.append(FONT_16.render("-" * 50 + f"{SYS_TURN_COUNT}턴" + "-" * 50, True, COL_GREEN))
                            if SYS_TURN_PLAYER == 1: #플레이어1의 차례일 경우
                                if sys_cursor_h == 0 or sys_cursor_h == 1 or sys_cursor_h == 4: # 연산량이 필요 없을 경우
                                    pc1_Doll.act_passiveBfSkill(pc2_Doll, sys_cursor_h, COL_RED, SYS_TURN_COUNT) # 턴 개시 시 패시브 발동
                                    ...
                                    if pc1_Doll.mp >= pc1_Doll.scost:
                                        ...
                                        SYS_BATTLE_LOG.append(FONT_22.render(f"{pc1_Doll.name}의 스킬을 사용할 연산력이 {pc1_Doll.scost - pc1_Doll.mp} 부족합니다.", True, COL_RED))
                                        sys_cursor_h = 0 # 커서 위치 초기화
                                    ...
                                    if pc1_Doll.mp >= pc1_Doll.ucost:
                                        ...
                                        SYS_BATTLE_LOG.append(FONT_22.render(f"{pc1_Doll.name}의 궁극기를 사용할 연산력이 {pc1_Doll.ucost - pc1_Doll.mp} 부족합니다.", True, COL_RED))
                                        sys_cursor_h = 0 # 커서 위치 초기화
                            elif SYS_TURN_PLAYER == 2: #플레이어2의 차례일 경우
                                if sys_cursor_h == 0 or sys_cursor_h == 1 or sys_cursor_h == 4: # 연산량이 필요 없을 경우
                                    pc2_Doll.act_passiveBfSkill(pc1_Doll, sys_cursor_h, COL_BLUE, SYS_TURN_COUNT) # 턴 개시 시 패시브 발동
                                    ...
                                    if pc2_Doll.mp >= pc2_Doll.scost:
                                        ...
                                        SYS_BATTLE_LOG.append(FONT_22.render(f"{pc2_Doll.name}의 스킬을 사용할 연산력이 {pc2_Doll.scost - pc2_Doll.mp} 부족합니다.", True, COL_BLUE))
                                        sys_cursor_h = 0 # 커서 위치 초기화
                                elif sys_cursor_h == 3: # 스킬 사용시 연산량 체크
                                    if pc2_Doll.mp >= pc2_Doll.ucost:
                                        ...
                                        SYS_BATTLE_LOG.append(FONT_22.render(f"{pc2_Doll.name}의 궁극기를 사용할 연산력이 {pc2_Doll.ucost - pc2_Doll.mp} 부족합니다.", True, COL_BLUE))
                                        sys_cursor_h = 0 # 커서 위치 초기화
                    else: # 키 입력으로 루프 탈출
                        if event.key == pygame.K_z:
                            loop_st2 = False
